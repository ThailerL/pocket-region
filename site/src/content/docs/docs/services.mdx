...
Each example is a taste of one service, not a list of what it supports: that's MiniStack's page
for it, linked from each section. Every Run boots a fresh region in this tab.
...
An item whose TTL has passed is removed within about a second. MiniStack removes expired items
from a background thread, which Pocket Region runs from a timer instead, as
[scheduled work](#scheduled-work) describes.
...
```js
import { createRegion, requestHandler } from 'pocket-region/browser';
import { DynamoDBClient, CreateTableCommand, UpdateTimeToLiveCommand, PutItemCommand, ScanCommand } from '@aws-sdk/client-dynamodb';
...
await dynamodb.send(new PutItemCommand({ TableName: 'sessions', Item: { id: { S: 'expired' }, expires: { N: String(now - 60) } } }));
await dynamodb.send(new PutItemCommand({ TableName: 'sessions', Item: { id: { S: 'live' }, expires: { N: String(now + 3600) } } }));

const ids = async () => (await dynamodb.send(new ScanCommand({ TableName: 'sessions' }))).Items.map((item) => item.id.S);
console.log('before:', await ids());
for (let tries = 0; tries < 10 && (await ids()).includes('expired'); tries++) {
  await new Promise((resolve) => setTimeout(resolve, 500));
}
console.log('after:', await ids());
