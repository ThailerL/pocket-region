...
Lambda preinstalls the AWS SDK. A page loads it for the handler, and Node finds it in your
...
Lambda. In a page, `process.env` is provided for these, and `fetch` calls to the region's port on
`localhost`, `127.0.0.1`, or `[::1]` are routed to the region. Every other `fetch` goes out as
normal. One exception: the SDK's browser build reads no environment, so a page handler that
