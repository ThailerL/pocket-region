...
| Bare imports, such as `@aws-sdk/client-s3` | Need the package in `node_modules` or bundled in | Load from jsDelivr, or from the page's import map when it names them. SDK clients get the region as their defaults |
| Imports from a full URL | Work | Work |
| Parent environment variables | Only `PATH` | None |

Lambda preinstalls the AWS SDK, and so does the page: a handler that imports it bare and creates a
client with no options runs there as written. In Node, bundle the SDK with your code.
