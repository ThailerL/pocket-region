...
| Bare imports, such as `@aws-sdk/client-s3` | Resolve from your project's `node_modules`, the nearest one above the working directory | Load from jsDelivr, or from the page's import map when it names them. SDK clients get the region as their defaults |
