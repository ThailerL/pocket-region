import { commandFor, type Clients, type SdkClient } from './dispatch.ts';
...
const command = (operation: string) => commandFor('s3api', operation);
...
type Verb = (client: SdkClient, rest: string[], files?: Files) => Promise<string[]>;

const list: Verb = async (client, [target]) => {
  if (!target) {
    const ListBuckets = await command('ListBuckets');
    const { Buckets } = await client.send(new ListBuckets({}));
    return formatBuckets(Buckets as Bucket[]);
  }
  const uri = parseS3Uri(target);
  if (!uri) throw new UsageError(`"${target}" is not an s3:// URI`, 's3');
  const ListObjectsV2 = await command('ListObjectsV2');
  const listing = await client.send(
    new ListObjectsV2({ Bucket: uri.Bucket, Prefix: uri.Key, Delimiter: '/' }),
  );
  return formatObjects(listing);
};

const copy: Verb = async (client, [from, to], files) => {
  ...
  if (!destination) {
    ...
    const GetObject = await command('GetObject');
    const { Body } = await client.send(new GetObject(source));
  ...
  if (source) {
    const CopyObject = await command('CopyObject');
    const CopySource = `${source.Bucket}/${source.Key}`;
    await client.send(new CopyObject({ Bucket: destination.Bucket, Key, CopySource }));
    return [`copy: ${from} to ${target}`];
  }
  const PutObject = await command('PutObject');
  const Body = await (await localFiles(files)).read(from);
  await client.send(new PutObject({ Bucket: destination.Bucket, Key, Body }));
  return [`upload: ${from} to ${target}`];
};

const makeBucket: Verb = async (client, [target]) => {
  const uri = parseS3Uri(target);
  if (!uri?.Bucket) throw new UsageError('mb needs an s3://bucket path', 's3');
  const CreateBucket = await command('CreateBucket');
  await client.send(new CreateBucket({ Bucket: uri.Bucket }));
  return [`make_bucket: ${uri.Bucket}`];
};

const remove: Verb = async (client, [target]) => {
  const uri = parseS3Uri(target);
  if (!uri?.Key) throw new UsageError('rm needs an s3://bucket/key path', 's3');
  const DeleteObject = await command('DeleteObject');
  await client.send(new DeleteObject(uri));
...
export async function runS3Verb(argv: string[], clients: Clients, files?: Files) {
  ...
  const lines = await run(await clients('s3api'), rest, files);
